...
r=3/5;
%控制器参数
k11=3;k12=3;k13=3;k21=3;k22=3;k23=3;
...
x21c=@(t)(0.6*cos(t));
%定义函数
H121=@(t)(t^2);H122=@(t)(t^2);H221=@(t)(t^2);H222=@(t)(t^2);

%系统初值
x11_0=0.2;x12_0=0.1;x13_0=0;
x21_0=0.3;x22_0=0.1;x23_0=0;
%滤波器初值
...
for i=1:N-1
    w1(i)=(2*(x11(i)-x11c(t(i))-xi11(i)))*(H121(x11(i))+H221(x11(i)))/((x11(i)-x11c(t(i))-xi11(i))*(x11(i)-x11c(t(i))-xi11(i))+psi1);
    w2(i)=(2*(x21(i)-x21c(t(i))-xi21(i)))*(H122(x21(i))+H222(x21(i)))/((x21(i)-x21c(t(i))-xi21(i))*(x21(i)-x21c(t(i))-xi21(i))+psi2);
    a11(i)=-k11*(x11(i)-x11c(t(i)))+0.8*cos(t(i))-s11*(x11(i)-x11c(t(i))-xi11(i))^r-rho1(i)*w1(i);
    ...
    a12(i)=-k12*(x12(i)-phi111(i))+v111(i)-(-0.8*x12(i)+sin(x12(i)))-(x11(i)-x11c(t(i)))-s12*(x12(i)-phi111(i)-xi12(i))^r;
    ...
    u1(i)=-k13*(x13(i)-phi121(i))+v121(i)-(-0.6*x12(i)+0.2*exp(-(x12(i))^2))-(x12(i)-phi111(i))-s13*(x13(i)-phi121(i)-xi13(i))^r;
    
    a21(i)=-k21*(x21(i)-x21c(t(i)))-0.6*sin(t(i))-s21*(x21(i)-x21c(t(i))-xi21(i))^r-rho2(i)*w2(i);
    ...
    a22(i)=-k22*(x22(i)-phi211(i))+v211(i)-(-0.7*x22(i)+sin(x22(i)))-(x21(i)-x21c(t(i)))-s22*(x22(i)-phi211(i)-xi22(i))^r;
    ...
    u2(i)=-k23*(x23(i)-phi221(i))+v221(i)-(-0.5*x22(i)-0.3*(x22(i))^2)-(x22(i)-phi211(i))-s23*(x23(i)-phi221(i)-xi23(i))^r;
    
    x11(i+1)=x11(i)+x12(i)*dt;
    x12(i+1)=x12(i)+(x13(i)-0.8*x12(i)+sin(x12(i))-0.5*(x11(i)-x21(i)))*dt;
    x13(i+1)=x13(i)+(u1(i)-0.6*x12(i)+0.2*exp(-(x12(i))^2))*dt;
    x21(i+1)=x21(i)+x22(i)*dt;
    x22(i+1)=x22(i)+(x23(i)-0.7*x22(i)+sin(x22(i))+0.5*(x11(i)-x21(i)))*dt;
    x23(i+1)=x23(i)+(u2(i)-0.5*x22(i)-0.3*(x22(i))^2)*dt;
end
figure;
plot(t,0.8*sin(t),'--b','Linewidth',2,'DisplayName','$0.8sin(t)$');
hold on;
...
legend;
figure;
plot(t,0.6*cos(t),'--b','Linewidth',2,'DisplayName','$0.6cos(t)$');
hold on;
